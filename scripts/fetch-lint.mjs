// Fetches the pinned m1-lint binary from its GitHub Release into server/.
// Replaces any manual `cp` of a locally-built binary.
//
// Version + repo are pinned in package.json under "m1": { lintRepo, lintVersion }.
// Auth: uses the `gh` CLI (honours GH_TOKEN) when available; m1-lint's repo is
// public, so an unauthenticated download also works.
//
// Usage:
//   node scripts/fetch-lint.mjs                 # current platform, pinned version
//   node scripts/fetch-lint.mjs --target x86_64-pc-windows-msvc
//   node scripts/fetch-lint.mjs --version v0.2.0 --out server
...
const repo = arg("repo", cfg.lintRepo);
const version = arg("version", cfg.lintVersion);
const target = arg("target", defaultTarget());
const outDir = path.resolve(root, arg("out", "server"));

if (!repo || !version) {
  console.error(
    "Missing pin: set package.json m1.lintRepo and m1.lintVersion (or pass --repo/--version).",
  );
  process.exit(1);
}

const isWindows = target.includes("windows");
const asset = `m1-lint-${target}${isWindows ? ".exe" : ""}`;
const outFile = path.join(outDir, `m1-lint${isWindows ? ".exe" : ""}`);
